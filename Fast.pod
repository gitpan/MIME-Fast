...
=head1 SYNOPSIS
  ...
  my $stream = MIME::Fast::Stream->new($data);
  my $msg = MIME::Fast::Parser::construct_message($stream, 1);
  $msg->set_subject( 'Re: ' . $msg->get_subject() );

  print 'Content-Type of message is ' .
    $msg->mime_part->get_content_type->to_string . "\n";

  $part = $msg->mime_part;
  $part = $part->get_subpart_by_number(0,3,1);
  ...
  $old_header = $header{'X-Info'};
  $header{'X-Info'} = [ @{$old_header}, 'New header line'];
...
=item I<construct_message>(text, preserve_headers = 1)

=item I<construct_message>(FILE, preserve_headers = 1)

=item I<construct_message>($mime_stream, preserve_headers = 1)
...
Returns the number of children parts in the multipart MIME::Fast::Part
...
=item I<get_subpart_by_number> (index, ...)
...
E.g.
    ...
    mime_part->get_subpart_by_number(0,2,1)
...
E.g.
    ...
    $msg = MIME::Fast::Parser->construct_message($stream, 1);
    tie %header, 'MIME::Fast::Hash::Header', $msg;
    $header{'From'} = 'John Doe <john@domain>';
...
Piotr Klaban, info@klaban.torun.pl

=head1 SEE ALSO

perl(1).

The homepage of gmime C library at http://www.xtorshun.org/gmime
